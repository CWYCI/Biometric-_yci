...
interface LateComersWidgetProps {
  lateComers: LateEmployee[];
  title?: string;
}

export function LateComersWidget({ lateComers = [], title = 'Today\'s Late Comers' }: LateComersWidgetProps) {
  return (
    <Card className="col-span-1 lg:col-span-2">
      <CardHeader>
        <CardTitle className="flex items-center gap-2">
          <Clock className="h-5 w-5 text-red-600" />
          {title} ({lateComers.length})
        </CardTitle>
      </CardHeader>
      <CardContent>
